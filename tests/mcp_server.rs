...
#[test]
fn test_stdio_cancels_in_flight_traversal_and_cleans_registry() {
    let count = 20_000u64;
    let nodes = (0..count)
        .map(|id| Node {
            id: NodeId(id),
    ...
    let edges = (0..count - 1)
        ...
        .collect::<Vec<_>>();
    let input = r#"{"jsonrpc":"2.0","id":100,"method":"tools/call","params":{"name":"graphia_dependency_path","arguments":{"from":"n0","to":"n19999","max_depth":20000}}}
{"jsonrpc":"2.0","method":"$/cancelRequest","params":{"id":100}}
{"jsonrpc":"2.0","id":101,"method":"ping"}
"#;
    let mut output = Vec::new();
    McpServer::new(None)
        .with_graph(Graph::new(nodes, edges))
        .run_stream(Cursor::new(input.as_bytes()), &mut output)
        .expect("stdio cancellation");
    ...
    let cancelled = responses
        .iter()
        .find(|response| response.id == RequestId::Number(100))
        .expect("cancelled response");
    assert_eq!(
        cancelled.error.as_ref().map(|error| error.code),
        Some(error_codes::CANCELLED)
    );
    assert!(
        responses
            .iter()
            .any(|response| { response.id == RequestId::Number(101) && response.error.is_none() })
    );
}
